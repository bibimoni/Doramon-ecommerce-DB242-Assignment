...
# -- buyers: buyerX, johnDoe123, janeDoe, user99
# -- Sample data for Cart
# INSERT INTO Cart (cart_id, buyer_usr) VALUES
#   (4, 'buyerX'    ),
#   (1, 'johnDoe123' ),
#   (2, 'janeDoe'),
#   (3, 'user99');
...
SELECT * FROM Buyer b JOIN Cart c WHERE c.buyer_usr = b.username;
SELECT * FROM Product p JOIN Variation v WHERE v.product_id = p.product_id;

CALL Proc_get_all_products();

-- Sample data for Cart_has_variations
...
DELETE FROM Cart_has_variations WHERE cart_id = 3 AND product_id = 3 AND variation_id = 3;